...
// A mirrored version of the pix_freq, splittion it on two sides starting from the middle
...
void tug_of_war() {
    ...
    int splitPosition = remap(f0, MIN_FREQUENCY, MAX_FREQUENCY, 0, virtual_led_count);
...
void random_raindrop(){
    int startIdx = random(virtual_led_count);

    leds[startIdx] = CHSV(fHue, 255, vbrightness);
    
    for(int i = virtual_led_count-1; i > 0; i--) {
      if (i != startIdx) {
        leds[i] = leds[i-1];
      }
    }
